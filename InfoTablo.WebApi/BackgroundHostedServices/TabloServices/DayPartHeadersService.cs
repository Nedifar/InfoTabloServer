        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        cache.Set("dayPart", "Memories", new MemoryCacheEntryOptions { AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(60) });
                    }
                    await _hubContext.Clients.All.SendAsync("SendDatPart", cache.Get("dayPart"));
                }
                catch(Exception ex)
                {
                    Console.WriteLine(ex.Message + "\tDayPartHeadersService");
