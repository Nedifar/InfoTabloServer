...
namespace TabloBlazorMain.Server.Controllers
{
    ...
    public class LastDanceController : ControllerBase
    {
        ...
        public LastDanceController(IHubContext<GoodHubGbl> hubContext, context _context, IMemoryCache cache)
        {
        ...
        public ActionResult GetGroupMobile(string group, DateTime date)
        {
            ...
            if (group == null)
            { return NotFound("Введите название группы."); }
            IXLWorksheet result = null;
        ...
        {
            if (teacher == null)
            { return NotFound("Введите имя преподавателя."); }
            var currentShedule = new Shedule(date);
            IXLWorksheet result = null;
        ...
        {
            if (cabinet == null)
            { return NotFound("Введите номер кабинета."); }
            var currentShedule = new Shedule(date);
            IXLWorksheet result = null;
            List<FullDayWeekClass> fullDayWeekClasses = new List<FullDayWeekClass>();
